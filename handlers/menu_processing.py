...
async def del_update_cart(session, user_id):
    carts = await orm_get_user_carts(session, user_id)
    for cart in carts:
        await orm_delete_from_cart(session, user_id, cart.product_id)


async def del_time_update_cart(session, user_id):
    carts = await orm_get_user_carts(session, user_id)
    current_time = datetime.datetime.now()

    for cart in carts:
        if (current_time - cart.updated).total_seconds() > 600:  # 600 секунд = 10 минут
            await orm_delete_from_cart(session, user_id, cart.product_id)
...
async def carts(session, level, menu_name, page, user_id, product_id):
    ...
    # await del_time_update_cart(session, user_id)
